...
head(fitted_models[,-2], n = 10) %>%
  mutate(complexity = round(complexity,2))
```
...
#first row: urine output
  plot(x = 0:1000, y = seq(0,4, length.out=1001),
       ...
       xlim   = c(0,1000), 
       ylim   = c(0, 4), 
       main = glue("GAM without Urine-24h and Creatinine (D\u00B2={dev_nocirc}%)"),
       ylab   = "", 
       xlab   = "",
       type  = "n")
  ...
  plot(gam_all, 
       ...
       ylim   = c(0, 4), 
       select = 1,
       main = glue("GAM with Urine-24h and Creatinine (D\u00B2={dev_withcirc}%)"),
       ylab   = "", 
       xlab   = "",
       shift  = shift_urine24)
  curve(jump_urine, 
...
# third row: artph
  ...
  plot(gam_all,
    ...
    abline(h=0, lty = 3)
    rug(unique(train_data$artph))
  mtext("Arterial pH", side = 4, line = 1)
...
# fourth row: bun
  ...
  plot(gam_all,
    ...
    abline(h=0, lty = 3)
    rug(unique(train_data$bun))
  mtext("Blood Urea Nitrogen", side = 4, line = 1)
